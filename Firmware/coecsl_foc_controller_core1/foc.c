...
void set_dtc(ControllerStruct *controller){
	...
	if(!PHASE_ORDER){
	    EPwm1Regs.CMPB.bit.CMPB = (int)(PWM_PERIOD*dtc_u);
        EPwm2Regs.CMPB.bit.CMPB = (int)(PWM_PERIOD*dtc_v);
        EPwm3Regs.CMPB.bit.CMPB = (int)(PWM_PERIOD*dtc_w);

	}
	else{
        EPwm2Regs.CMPB.bit.CMPB = (int)(PWM_PERIOD*dtc_u);
        EPwm1Regs.CMPB.bit.CMPB = (int)(PWM_PERIOD*dtc_v);
        EPwm3Regs.CMPB.bit.CMPB = (int)(PWM_PERIOD*dtc_w);
	}
}
...
void analog_sample (ControllerStruct *controller){
    ...
    uint16_t adcAResult = AdcaResultRegs.ADCRESULT0;
    uint16_t adcBResult = AdcbResultRegs.ADCRESULT0;
    uint16_t adcCResult = AdccResultRegs.ADCRESULT0; // zero ~ 2360
...
void abc( float theta, float d, float q, float *a, float *b, float *c){
    ...
    float cf = cos_lut(theta); // TODO: compare this execution time to cosf and sinf
...
void zero_current(ControllerStruct *controller){
    ...
    set_dtc(controller);
    int i;
    for (i = 0; i<n; i++){               // Average n samples
...
void init_controller_params(ControllerStruct *controller){
    ...
    int i;
    for(i = 0; i<128; i++)	// Approximate duty cycle linearization
    {
